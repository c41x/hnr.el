...
(defun hnr--process-get-item (res)
  ...
  (let* ((item-json (json-read-from-string res))
	 (item-id (or (cdr (assoc 'id item-json)) 0))
	 (item-type (or (cdr (assoc 'type item-json)) "error"))
	 (item-descendants (or (cdr (assoc 'descendants item-json)) "error"))
	 (item-url (or (cdr (assoc 'url item-json)) "error"))
	 (item-by (or (cdr (assoc 'by item-json)) "error"))
	 (item-kids (or (cdr (assoc 'kids item-json)) "error"))
	 (item-time (or (cdr (assoc 'time item-json)) 0))
    ...
    (insert (propertize (format "%s %-4d " (make-string 1 9650) (or (cdr (assoc 'score item-json)) 0))
			'id item-final-url
			'root item-id
			'kids item-kids
			'face 'hnr-points-face))
    (hnr--insert-primary (or (cdr (assoc 'title item-json)) "error"))
    (newline)
    (beginning-of-line)
